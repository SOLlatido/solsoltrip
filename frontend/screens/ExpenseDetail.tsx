...
const ExpenseDetail = ({route}) => {
        ...
        {friendsList.map((friendName) => (
          <View key={friendName} style={tw`flex-row justify-between items-center mb-3`}>
            <View style={tw`flex-row items-center`}>
              <Text style={tw`text-lg mr-2`}>{friendName}</Text>
              <TextInput
                style={tw`border-[0.3] border-[#ddd] rounded-2 h-13 px-3`}
            ...
            <View style={tw`flex-row items-center`}>
              ...
              <View
                style={[
                  tw`w-6 h-6 ml-2 rounded-full border-2`,
  ...
  return (
    ...
    <View style={tw `flex-1`}>
      <Text style={tw `mb-4 mt-4`}>사진 등록(선택)</Text>
      {imageSource ? (
        <Image
          source={imageSource}
    ...
    <View style={tw `flex-1 mt-5`}>
      <Text style={tw `mb-4`}>지출에 참여한 동행</Text>
